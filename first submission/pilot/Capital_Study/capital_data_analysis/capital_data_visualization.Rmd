...
Fig. 5: The graph shows a density plot of the data displaying the median log-transformed initiation time (ini_latency_median) for each trial type (dashed lines) and the distribution of log-transformed initiation times for each trial type of each trial (colored density distribution for respective trial type). The log initiation time values are differently distributed. The peak of Typical values is remarkably higher than the peak of Atypical values. For the Typical values there is a small peak on the left for small initiation time values. This might indicate a bimodel/multimodal distribution of values (view Bimodality check in animal_data_cleaning!). The distribution for Atypical values is more spread out. The median for Atypical values is slightly larger than the median for Typical values. This difference might not be noteworthy. 
...
Fig. 6: The graph shows a density plot of the data displaying the median traveled distance (travel_dis__median) for each trial type (dashed lines) and the distribution of traveled distance for each trial type of each trial (colored density distribution for respective trial type). Both distributions have a peak between [0,100], whereas the peak for Typical values is remarkably higher than the peak for Atypical values. The distribution for Atypical values is more spread out.  The distribution of Typical values shows several peaks, which indicates multimodality. The median for Atypical values is larger than the median of Typical values. This difference might however not be noteworthy.   
